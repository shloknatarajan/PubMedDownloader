...
class PubMedDownloader:
    ...
    def local_html_to_markdown(self, save_dir: str = "data/", overwrite: bool = False) -> None:
        """
        ...
        if not overwrite:
            ...
            htmls = [html for html in htmls if html.replace('.html', '') not in existing_markdown]
            html_paths = [os.path.join(html_dir, f) for f in htmls]
            
        logger.info(f"Converting {len(htmls)} HTML files to Markdown")
        for html_path in tqdm(html_paths, desc=f"Converting html ({save_dir}/html) to markdown"):
    ...
    def pmids_to_markdown(self, pmids: List[str], save_dir: str = "data", overwrite: bool = False) -> None:
        """
...
def convert_pmids_from_file(file_path: str, save_dir: str = "data", overwrite: bool = False):
    """
...
if __name__ == "__main__":
    ...
    parser.add_argument("--file_path", type=str, help="Path to the txt file containing PMIDs")
    parser.add_argument("--save_dir", type=str, default="data", help="Directory to save the files to (default: 'data/')")
    parser.add_argument("--overwrite", action="store_true", help="Whether to overwrite existing markdown files (default: False)")
