...
class PubMedHTMLToMarkdownConverter:
    ...
    def _handle_scanned_document(self) -> str:
        ...
        lines = []
        lines.append("*Note: This is a scanned document with limited structured text. Full content available in PDF.*")
        lines.append("")
        ...
        if subsections:
            ...
            for element in abstract_section.descendants:
                if isinstance(element, Tag):
                    if element.name in ['h3', 'h4'] and 'pmc_sec_title' in element.get('class', []):
        ...
        for element in section.children:
            if isinstance(element, Tag):
                if element.name == 'p':
                    ...
                    elif any(cls in element.get('class', []) for cls in ['fig', 'figure']):  # Figure
    ...
    def _convert_table_to_markdown(self, table_tag: Tag) -> str:
        ...
        if rows:
            ...
            lines.append("| " + " | ".join(["-" * max(1, len(cell)) for cell in header]) + " |")
...
def main():
    ...
    if len(sys.argv) != 3:
        logger.error("Usage: python html_to_markdown.py <input_html_file> <output_md_file>")
        sys.exit(1)
